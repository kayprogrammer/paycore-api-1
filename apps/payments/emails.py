...
class PaymentEmailUtil:
    ...
    def send_invoice_email(cls, invoice):
        ...
        try:
            ...
            items_list = list(InvoiceItem.objects.filter(invoice=invoice).values(
            ...
            context = {
                ...
                'tax_amount': f"{invoice.tax_amount:,.2f}" if invoice.tax_amount else None,
                'discount_amount': f"{invoice.discount_amount:,.2f}" if invoice.discount_amount else None,
    ...
    def send_payment_received_email(cls, payment):
        ...
        try:
            ...
            logger.error(f"Failed to send payment received email for {payment.reference}: {e}", exc_info=True)
